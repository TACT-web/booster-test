...
if not st.session_state.agreed:
    ...
    if files:
        ...
        if saved_data:
            # ファイル名の形式 history_学校_学年.json から復元
            ...
            if len(parts) == 3:
                ...
                st.session_state.current_tab = "履歴" # 履歴があれば履歴をまず見せる
...
if st.session_state.current_tab == "学習":
    if st.session_state.review_mode:
        ...
        if cam_file and st.button("✨ ブースト開始", use_container_width=True):
            if not st.session_state.user_api_key: st.error("新規解析にはAPIキーが必要です。サイドバーで設定してください。")
            else:
    ...
    if st.session_state.final_json:
        ...
        # (ここから下のクイズ表示・解説表示ロジックはV1.2を完全継承)
        ...
        for i, q in enumerate(res.get("quizzes", [])):
            ans = st.radio(f"問{i+1}: {q['question']}", q['options'], key=f"q_{i}_{st.session_state.review_mode}", index=None)
            if ans:
                answered += 1
        ...
        if answered == len(res.get("quizzes", [])) and len(res.get("quizzes", [])) > 0:
            if st.button("🏁 終了" if st.session_state.review_mode else "🏁 結果を保存"):
                rate = (score / len(res["quizzes"])) * 100
                st.balloons() if rate == 100 else None
                st.header(f"スコア: {rate:.0f}%")
                if not st.session_state.review_mode:
                    subj = res["used_subject"]
                    if subj not in st.session_state.history: st.session_state.history[subj] = []
                    st.session_state.history[subj].append({"date": datetime.datetime.now().strftime("%m/%d %H:%M"), "page": res.get("page", "?"), "score": f"{rate:.0f}%", "quizzes": res["quizzes"]})
        ...
        with st.expander(f"📙 {sub} ({len(logs)}件)"):
            for i, log in enumerate(logs):
                ...
                if c3.button("🔄 解き直し", key=f"rev_{sub}_{i}"):
                    ...
                    st.session_state.current_tab = "学習" # ここで学習画面へジャンプ
                    st.rerun()
